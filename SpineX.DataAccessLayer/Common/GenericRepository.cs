...
using SpineX.DataAccessLayer.Interface.Common;

namespace SpineX.DataAccessLayer.Common
{
    public class GenericRepository<TObject> : IGenericRepository<TObject> where TObject : class
    {
        ...
        public TObject Update(TObject updated, int key)
        {
            ...
            var existing = _context.Set<TObject>().Find(key);
            if (existing != null)
            {
        ...
        public async Task<TObject> UpdateAsync(TObject updated, int key)
        {
            ...
            var existing = await _context.Set<TObject>().FindAsync(key);
            if (existing != null)
            {
                _context.Entry(existing).CurrentValues.SetValues(updated);
                await _context.SaveChangesAsync();
